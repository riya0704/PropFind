...
export function PropertySearchForm() {
    return (
        <div className="bg-background/80 backdrop-blur-sm p-6 rounded-2xl shadow-lg w-full max-w-5xl mx-auto border">
            <div className="flex flex-col lg:flex-row items-center justify-between gap-4 mb-4">
                <div className="relative w-full">
                    <MapPin className="absolute left-4 top-1/2 -translate-y-1/2 h-5 w-5 text-muted-foreground" />
                    <Input placeholder="Search Location..." className="w-full h-14 pl-12 pr-20 text-base rounded-full border-0 focus-visible:ring-0" />
                    <Button variant="ghost" size="icon" className="absolute right-2 top-1/2 -translate-y-1/2 h-10 w-10 rounded-full">
                        <Search className="h-5 w-5" />
                    </Button>
                </div>
                <Button size="lg" variant="outline" className="h-14 text-base rounded-full bg-white hover:bg-neutral-100 whitespace-nowrap">
                    List Your Property
                </Button>
            </div>
